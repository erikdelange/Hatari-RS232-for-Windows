...
#ifdef WIN32                    /* use of COM ports applies only to Windows targets */
#define ENABLECOMPORT           /* if defined enable use of Windows COM ports for serial connection */
                                /* #undef ENABLECOMPORT to use the original Hatari functionality */
#ifdef ENABLECOMPORT
#include <ctype.h>
#include <windows.h>            /* header for Win32 API functions */
#define COMPORT "COM9"          /* default COM port to use for serial connection */
bool UseComPort = true;         /* true to use comport, false to use original Hatari functionality */
char ComPortName[16];           /* COM port name including backslashes and dot */
#endif /* ENABLECOMPORT */
#endif /* WIN32 */
...
struct RS232_s {
#ifdef ENABLECOMPORT
	HANDLE fh; /* Win32 file handle, used for both read and write */
#endif /* ENABLECOMPORT */
...
static bool RS232_OpenCOMPort(void)
{
	...
	if (UseComPort) {
		if (RS232_MFP.fh == INVALID_HANDLE_VALUE) {
			RS232_MFP.fh = CreateFile(ComPortName,              /* name of the COM port */
			...
			if (RS232_MFP.fh == INVALID_HANDLE_VALUE) {
				Log_Printf(LOG_WARN, "RS232: Error %lu opening Windows serial port %s\n", GetLastError(), ComPortName);
				return false;
			}

			Log_Printf(LOG_INFO, "RS232: Successfully opened Windows serial port %s\n", ComPortName);

			SecureZeroMemory(&dcb, sizeof(DCB));
			dcb.DCBlength = sizeof(DCB);

			/* default settings for a serial port are 8N1 */
			if (GetCommState(RS232_MFP.fh, &dcb)) {
				dcb.BaudRate = CBR_9600;
				dcb.ByteSize = 8;
				dcb.Parity = NOPARITY;
				dcb.StopBits = ONESTOPBIT;
				if (SetCommState(RS232_MFP.fh, &dcb)) {
					Log_Printf(LOG_INFO, "RS232: default COM port settings: baudrate=%lu, databits=%d, stopbits=%d, parity=%d\n",
...
static void RS232_CloseCOMPort(void)
{
	...
	if (UseComPort) {
		if (RS232_MFP.fh != INVALID_HANDLE_VALUE) {
			CloseHandle(RS232_MFP.fh);
			RS232_MFP.fh = INVALID_HANDLE_VALUE; /* invalid handle indicates COM port is not open */
			Log_Printf(LOG_INFO, "RS232: Closed Windows serial port\n");
		}
		Dprintf(("Closed RS232 serial port.\n"));	
...
void RS232_Update(void)
{
	...
	if (UseComPort) {
		if (!RS232_MFP.ByteReceived && RS232_MFP.fh != INVALID_HANDLE_VALUE) {
			ClearCommError(RS232_MFP.fh, &Errors, &Stat); /* get info on size of input queue */
			if (Errors)
				Dprintf(("RS232: CommError: 0x%lx\n", Errors));

			if (Stat.cbInQue > 0) { /* something to read? */
				if (ReadFile(RS232_MFP.fh, &c, 1, &NumberOfBytesRead, NULL)) { /* read 1 byte */
					Dprintf(("RS232: ReadFile %4d 0x$%02x (%c)\n", c, c, isprint(c & 0x7F) ? c & 0x7F : '-'));
					...
					Dprintf(("RS232: ReadFile error %lu\n", GetLastError()));
			}
		}
		return;
	}
...
void RS232_Init(void)
{
	...
	if (initialized == false) {
		RS232_MFP.fh = INVALID_HANDLE_VALUE; /* only on startup no serial port is open */
		initialized = true;
	}

	/* If run-time configuration has changed  ... */
	if (RS232_MFP.fh != INVALID_HANDLE_VALUE)
		RS232_CloseCOMPort(); /* ... close comport before opening a new one */

	/* create comport name according to Windows standards (with backslashes and dot) */
...
static void RS232_HandleUCR(int16_t ucr)
{
	...
	if (UseComPort) {
		...
		Dprintf(("RS232_HandleUCR(%02x) : databits=%i, stop bits=%i, parity=%d even parity=%d\n", 
			ucr, nCharSize, nStopBits, bUseParity, bEvenParity));
		...
		if (RS232_MFP.fh != INVALID_HANDLE_VALUE) {
			...
			if (GetCommState(RS232_MFP.fh, &dcb)) {
				...
				SetCommState(RS232_MFP.fh, &dcb);
			} else
				Log_Printf(LOG_WARN, "RS232_HandleUCR: failed to set configuration bits\n" );
		}
		return;
	}
...
static bool RS232_SetBaudRate(int nBaud)
{
	...
	int i;
	DCB dcb;
	static const DWORD baudtable[][2] = { 
		{110, CBR_110},
		{300, CBR_300},
		{600, CBR_600},
		{1200, CBR_1200},
		...
		{128000, CBR_128000},
		{256000, CBR_256000},	
		{0, 0} /* 0,0 must be last in table */
	}; /* these are the baud rates supported by the Win32 API */
	DWORD BaudRate = -1;

	if (UseComPort) {
		for (i = 0; baudtable[i][0] != 0; i++) {
			if (baudtable[i][0] == (DWORD)nBaud) {
				BaudRate = baudtable[i][1];
				break;
			}
		}

		if (BaudRate == (DWORD)-1) {
			Dprintf(("RS232: Unsupported baud rate %d\n", nBaud));
			return false;
		}
