...
Only file *rs232.c* - taken from the Hatari version 2.6.1 repository - is modified, and only lines have been added (to verify this do 'diff' with *rs232_original.c* which is the unchanged version).

The modified code will only be present in a Windows build of Hatari (checked via existing macro WIN32) and only if (new) macro ENABLECOMPORT has been defined in file *rs232.c*.
...
To use this functionality you have to build your own version of Hatari. For this I used WSL and the instructions found here: http://clarets.org/steve/projects/20201220_building_hatari_for_windows.html. These worked perfectly except that LibSDL has moved to GitHub. And of course don't forget to replace file *rs232.c* with the one from this repository.
